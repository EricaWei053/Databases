...
graph = Graph("bolt://127.0.0.1:7474", auth=("neo4j", "WCCxb960503"))
...
class FanGraph(object):
    ...
    def __init__(self, auth=('neo4j', 'WCCxb960503'), host='localhost', port=7474, secure=False, ):
    ...
    def create_comment(self, uni, comment, team_id=None, player_id=None):
        ...
        """
        if uni is None or comment is None or (plaer_id is None and team_id is None):
        ...
        try:
            ...
            if tx:
                tx.rollback()
            raise RuntimeError("create_comment: Something wrong, e=", str(e))
    ...
    def create_sub_comment(self, uni, origin_comment_id, comment):
        ...
        try:
            ...
            if origin_comment is None:
                raise ValueError("create_sub_comment: origin_comment node not found.")
    ...
    def get_sub_comments(self, comment_id):
        ...
        """
        q = "match (c:Comment {comment_id: {cid}})<-[response:RESPONSE_TO]-"
...
def t1():
    q = "match (p:Player {last_name: {ln}) return p"
    res = FG.run_q(q, {"ln": "Fregson"})
    print("Result = ", res)
